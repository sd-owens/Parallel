...
main( int argc, char *argv[ ] )
{
	...
	size_t globalWorkSize[4] = { NUM_ELEMENTS, 1, 1 };  // might need to be 1, 1, 1
	...
	// 12a combine individual thread returns into combined sum.
    float sum = 0;
	...
	// 12b did it work?  Calculate MultiplySum without GPU and reduction.
	...
	// {
	// 	fprintf( stderr, "Results(vals): MultSum wrongly produced %13.6f instead of %13.6f (%13.8f)\n",
	// 		sum, expected, fabs(sum-expected) );

	// 	fprintf( stderr, "Results(bits): MultSum wrongly produced    0x%08x instead of    0x%08x\n",
	// 		LookAtTheBits(sum), LookAtTheBits(expected) );
	// }	

	fprintf( stderr, "%8d\t%4d\t%10d\t%10.3lf\t%10.3lf\tGigaMult-ReductionsPerSecond\n",
		NUM_ELEMENTS, LOCAL_SIZE, NUM_WORK_GROUPS, sum, (double)NUM_ELEMENTS/(time1-time0)/1000000000. );
